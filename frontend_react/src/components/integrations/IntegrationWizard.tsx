...
import { MessageCircle, CheckCircle, AlertTriangle, Key, Link as LinkIcon, User, Store } from 'lucide-react';
...
export const IntegrationWizard: React.FC<IntegrationWizardProps> = ({ onSuggestClose, onRefresh }) => {
        ...
        const loadTenants = async () => {
            try {
                ...
                if (Array.isArray(data)) {
                    setTenants(data);
                    // Optional: Default to first tenant if none selected?? No, better let user choose or default to 'Global' if null
                }
            } catch (e) {
    ...
    useEffect(() => {
        const loadConfig = async () => {
            ...
            try {
                ...
                const data = await fetchApi(`/admin/integrations/${provider}/config${query}`);
    ...
    const handleSave = async () => {
        ...
        try {
            await fetchApi(`/admin/integrations/${provider}/config`, {
                method: 'POST',
