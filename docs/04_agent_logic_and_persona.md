...
- **Horarios del Tenant por Día:** `check_availability` lee `tenants.working_hours` para determinar los horarios operativos del día consultado (en vez de usar env vars globales `CLINIC_HOURS_START/END`). Si el día está deshabilitado en la configuración, la tool responde que la clínica no atiende ese día. Soporta múltiples slots por día (ej. mañana y tarde con pausa al mediodía).
- **Sede por Día (Multi-Sede):** La respuesta de `check_availability` incluye la sede, dirección y link de Maps correspondiente al día consultado (resuelto desde `tenants.working_hours[day].location`). Si el día no tiene sede específica, usa la dirección general del tenant como fallback. `book_appointment` también incluye la sede en el mensaje de confirmación.
...
| `check_availability` | `date_query, [professional_name], [treatment_name], [time_preference]` | Consulta huecos libres para un día usando los **horarios del tenant por día** (`tenants.working_hours`). Llamar **una sola vez** por pregunta. Si piden "a la tarde" o "por la mañana", pasar `time_preference='tarde'` o `'mañana'`. La tool devuelve **rangos** (ej. "de 12:00 a 17:00") + **sede/dirección/Maps** del día. El agente no debe repetir el mensaje ni dar variaciones. |
