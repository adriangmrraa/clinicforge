...
class ChatwootAdapter(ChannelAdapter):
    ...
    async def normalize_payload(self, payload: Dict[str, Any], tenant_id: int) -> List[CanonicalMessage]:
        ...
        # Return canonical message to _process_canonical_messages pipeline
        # which handles: DB persistence, Socket.IO, deduplication, BufferManager enqueue
        self._log_normalization("Chatwoot", 1, [nexus_channel])
        return [canonical]
