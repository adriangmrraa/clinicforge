...
async def _process_canonical_messages(messages, tenant_id, provider, background_tasks):
    ...
    for msg in messages:
        ...
        try:
            ...
            # un comando de la Dra. debe entrar SIEMPRE (aunque su número esté silenciado o
            # bloqueado). Gateado por tenants.config->'staff_task_phones' dentro del helper, así
            # un PACIENTE que escriba "tarea" NUNCA carga nada (no está en la lista → pasa de largo
            # y sigue el flujo normal). Cada mensaje entra por separado → cada "Tarea:" es su propio
            # pendiente. Pedido Carlos 2026-07-24 (bug: la derivación activó override 24h y silenció
            # las "Tarea:" siguientes antes de llegar al interceptor del buffer).
            try:
                ...
                if await _maybe_create_staff_task(
                    ...
                    continue  # pendiente creado + confirmación enviada; no encolar ni correr agente
