    ...
    try:
        ...
        if patient_context and "HISTORIAL: Paciente recurrente" in patient_context:
            patient_context += (
                ...
                "4) Si menciona CONTINUAR/TERMINAR un tratamiento en curso ('tengo un implante a terminar', 'sigo con el tratamiento'): es CONTINUIDAD, no una evaluación nueva — ⛔ NO uses la plantilla 'en implantes lo ideal es hacer primero una evaluación' (eso es para pacientes NUEVOS que consultan por implantes; este ya ESTÁ en tratamiento). NO cotices la consulta de evaluación. Llamá check_availability YA y ofrecé opciones con SU profesional de siempre en el MISMO mensaje — sin pedir permiso ('¿querés que busque?' está PROHIBIDO: buscá y ofrecé). Los valores de su plan los maneja la clínica según su tratamiento. "
...
"""
        ...
        # P4 (caso Rodrigo): exponer el mensaje CRUDO del turno a book_appointment para el
    ...
    try:
        from services.inyecciones_frescas import (
            candado_issn_anti_ceder as _issn_ac_fn,
            candado_issn_no_deriva as _issn_fn,
        )
        ...
        # ANTI-CEDER (casos prod 21/07): el bot "dice que sí" ante "¿lo cubre ISSN?" →
        # forzar la aclaración correcta (cirugía maxilofacial→CIMO, el resto particular).
        _issn_ac_pre = response_text
        response_text = _issn_ac_fn(response_text, patient_context or "", _issn_last)
