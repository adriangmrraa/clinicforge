                ...
                <div className="flex items-center gap-1.5">
                    ...
                    {evt.payment_status === 'paid' && (
                        <div className="flex items-center gap-1">
                            ...
                            <span className="text-[11px] font-semibold text-emerald-400">{t('agenda.payment_paid')}</span>
                        </div>
                    )}
                    {evt.payment_status === 'partial' && (
                        <div className="flex items-center gap-1">
                            <div className="w-2 h-2 rounded-full bg-amber-400" />
                            <span className="text-[11px] font-semibold text-amber-400">{t('agenda.payment_partial')}</span>
                        </div>
                    )}
                    {evt.payment_status === 'pending' && evt.billing_amount > 0 && (
                        <div className="flex items-center gap-1">
                            <div className="w-2 h-2 rounded-full bg-red-400 animate-pulse" />
                            <span className="text-[11px] font-semibold text-red-400">{t('agenda.payment_pending')}</span>
                        </div>
                    )}
