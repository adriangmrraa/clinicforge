...
|---|---|---|---|---|---|
| 2026-07-03 | `d31e5d5`+`815c416`+`dd4d536`+`e6441db`+`f39c67f` | main.py · agendamiento (offer==book) + PASO 3 profesional interno + RAZONAMIENTO DE FECHA (preferred_days / días de atención / excepción consulta) | Paquete de 5 arreglos de agendamiento promovidos a PROD (prod-equivalentes de a5801eb/1ec9d91/7487f9c/db37b52/9e4b8ae): (1) la búsqueda multi-día respeta el profesional del día-semilla (offer==book, mata el pisado/loop); (2) el profesional es INTERNO (no nombra a Eli, no ofrece opciones); (3) respetar día de semana pedido vía preferred_days; (4) si ningún día pedido tiene agenda → explicar los días de atención (Opción A, no cambiar de profesional); (5) EXCEPCIÓN consulta general → Laura en días que Elizabeth no atiende (lun/jue). Verificado adversarialmente (workflows). NO validado en pruebas (deploy de pruebas estaba en código viejo) → Carlos valida en prod | Casos Matías/Lucas: pisado de profesional + loop, nombraba a Eli, ignoraba días pedidos. Ver [[clinicforge-agendamiento-bugs]] | `git revert f39c67f e6441db dd4d536 815c416 d31e5d5` |
| 2026-07-03 | `b724824` | main.py · F4/M1 guard + GREETING branch B + F5 PRIORIDAD **+** buffer_task.py · NET particular-nonsense (post [LOCAL_IMAGE]) | (1) NET determinista: borra la frase autocontradictoria "No trabajamos de forma directa/directamente con particular..." + cláusula de reintegro colgante (cubre los 2 templates: F4/M1 y check_insurance_coverage; corta por punto, salto de línea o emoji); NUNCA toca la línea legítima con OS real (ancla en "con particular"). (2) PROMPT: F4/M1 pasa de aviso blando a ⛔ regla dura (esa frase es SOLO para OS real not_found/rejected); GREETING branch B pregunta la cobertura tras presentarse (no repregunta si ya la dijo); F5 deja de arrancar con "PRECIO PRIMERO" para que M0 (cobertura) vaya primero si no se sabe | Caso Lucas: ante "cuánto sale una extracción" (sin OS) el bot decía "No trabajamos de forma directa con particular, así que sería particular" (sinsentido). Validado en pruebas (ahora pregunta cobertura). Prod-equivalente de `76b0723` | `git revert b724824` |
| 2026-07-03 | `ab62fa5` | main.py · GESTIÓN PREVIA (excepción presupuesto) + ancla | Acotar "si payment_status devuelve presupuesto → informalo": informar SOLO si es consulta genérica de saldo o si el NOMBRE coincide con lo que preguntó; si devuelve otro tratamiento → PROHIBIDO volcar montos (fuga) → derivar | Caso Luna (PROD): preguntó por el presupuesto del escaneo y el bot le volcó el de ORTOGNÁTICA ($2000). Prod-equivalente de `4f36dca` | `git revert ab62fa5` |
...
| 2026-07-02 | `d01be7c` | main.py · REGLA GESTIÓN PREVIA SIN REGISTRO (~11484, "NO aplica si") — guardarraíl afirma+agenda | Si el paciente AFIRMA que algo ya llegó/se lo dieron Y quiere SACAR TURNO ("llegaron mis placas, quiero turno", "me dieron la autorización, quiero agendar") → NO derivar, seguir el agendamiento normal. La derivación es SOLO cuando PREGUNTA por el estado de algo que no figura | Pedido de Carlos: distinguir preguntar-estado (derivar) de afirmar+querer-turno (agendar) | `git revert d01be7c` |
