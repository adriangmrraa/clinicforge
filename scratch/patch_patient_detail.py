...
# Add import
import_stmt = "import { ODONTOGRAM_STATES } from '../constants/odontogramStates';\n"
if "import { ODONTOGRAM_STATES }" not in content:
    content = content.replace("import Odontogram from '../components/Odontogram';", "import Odontogram from '../components/Odontogram';\n" + import_stmt)

# Replace getAffectedTeeth and renderOdontogramCard
...
# Use regex to find and replace the whole block from getAffectedTeeth to the end of renderOdontogramCard
pattern = re.compile(r'  /\*\* Extract affected tooth numbers from the structured diff diagnosis text \*/.*?  };\n', re.DOTALL)
content = pattern.sub(lambda m: new_render + '\n', content)
