...
7. Soporta dentición permanente (32 piezas) y temporal/decidua (20 piezas).""",
    ...
    },
    {
        "type": "function",
        "name": "ver_pendientes",
        "description": "Lista los PENDIENTES/TAREAS de la clínica con el vencimiento ya calculado. USALO para responder 'cuáles pendientes tengo', 'cuáles vencieron', 'cuáles están por vencer', 'cuáles urgentes', 'los más recientes'. NO uses obtener_registros para pendientes (el filtro de fecha falla).",
        "parameters": {
            "type": "object",
            "properties": {
                "filtro": {
                    "type": "string",
                    "enum": ["resumen", "vencidas", "por_vencer", "urgentes", "recientes"],
                    "description": "Qué mostrar: 'resumen' (default: vencidos + por vencer), 'vencidas' (ya vencieron), 'por_vencer' (futuras aún sin vencer), 'urgentes' (prioridad urgente), 'recientes' (últimas cargadas).",
                }
            },
        },
    },
    ...
    # -------------------------------------------------------------------------
    {
        "type": "function",
        "name": "enviar_email",
        "description": "Envía un email con el resumen de una conversación de chat. Puede enviar a todos los profesionales/CEOs/secretaries de la clínica o a un email personalizado. Incluye el contexto del paciente, la conversación reciente y próximo turno.",
        "parameters": {
            "type": "object",
            "properties": {
                "phone_number": {
                    "type": "string",
                    "description": "Teléfono del paciente/conversación para generar el resumen (obligatorio)",
                },
                "destinatario": {
                    "type": "string",
                    "description": "Email de destino personalizado. Si no se provee, envía a todos los profesionales, CEOs y secretaries de la clínica.",
                },
                "asunto": {
                    "type": "string",
...
async def _enviar_email_chat(args: Dict, tenant_id: int, user_role: str) -> str:
    ...
    try:
        ...
        messages_rows = await db.pool.fetch(
            ...
            "WHERE from_number = $1 AND tenant_id = $2 ORDER BY created_at DESC LIMIT 20",
            phone_number, tenant_id,
        )

        chat_history_html = ""
        for msg in reversed(messages_rows):
            direction = "Paciente" if msg["role"] == "user" else "Clínica"
            chat_history_html += f"<p><strong>{direction}:</strong> {msg['content']}</p>"

        next_apt = await db.pool.fetchrow(
            "SELECT a.appointment_datetime, a.appointment_type, p.first_name as prof_name "
            "FROM appointments a JOIN professionals p ON a.professional_id = p.id "
            "WHERE a.patient_id = $1 AND a.tenant_id = $2 AND a.status NOT IN ('cancelled', 'completed') "
            "AND a.appointment_datetime > NOW() ORDER BY a.appointment_datetime ASC LIMIT 1",
        ...
        if not destinatario:
            ...
            if not destinatarios:
                return "No hay profesionales o CEOs con email configurado en esta clínica."
        else:
        ...
        <p><strong>Obra Social:</strong> {patient_row.get('insurance_provider') or 'No especificada'}</p>
        ...
        try:
            ...
            return f"✅ Email enviado correctamente a: {', '.join(destinatarios)}"
        except Exception as e:
...
async def execute_nova_tool(
    ...
    try:
        ...
        # Meta-tool: proxy for voice to access all 77 tools
            ...
            return await execute_nova_tool(
                inner_name, inner_args, tenant_id, user_role, user_id
            )
...
async def _ver_pendientes(args: Dict, tenant_id: int) -> str:
    ...
    de fecha del tool genérico, que fallaba). Pedido Carlos 2026-07-24. Tenant-scoped, fail-safe."""
    from db import db
    ...
    try:
        ...
        # resumen (default): total + vencidos + por vencer
