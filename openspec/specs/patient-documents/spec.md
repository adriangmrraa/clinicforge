# Spec: Patient Documents Migration

**Change:** `link-chat-unique-violation-fix`
**Date:** 2026-06-04
**Status:** Approved
**Type:** Backend (Migration / Document Management)
**Priority:** P1 (Prevent unique constraint violations on chat linking)

---

## 1. Overview

When migrating media attachments from chat messages to a patient's documents, the system must ensure data integrity and prevent unique constraint violations on `(tenant_id, patient_id, file_name)`. This specification defines the requirements for generating unique filenames for linked chat attachments and tracking individual attachment migration status.

## 2. Requirements

### Requirement: Unique Filename Generation on Chat Linking

The system MUST ensure that every saved file has a unique filename for that patient and tenant.
