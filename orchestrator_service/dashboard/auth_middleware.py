...
class CEOSessionManager:
    """Gestor de sesiones CEO — respaldado por Redis."""
    ...
    async def create_session(self, request: Request) -> str:
        """Crea una nueva sesión CEO y la persiste en Redis."""
        ...
        try:
            ...
            if redis is not None:
                await redis.set(
                    f"dashboard_session:{session_id}",
                    json.dumps(session_data),
                    ex=SESSION_TTL_SECONDS,
                )
            else:
                logger.warning("Redis unavailable — CEO session created but NOT persisted (in-memory fallback disabled)")
        except Exception as e:
            logger.warning(f"Redis error creating CEO session {session_id[:8]}...: {e}")

        logger.info(f"✅ Nueva sesión CEO creada: {session_id[:8]}...")
        return session_id

    async def validate_session(self, session_id: str) -> bool:
        """Valida una sesión CEO leyendo desde Redis."""
        try:
            redis = get_redis()
            if redis is None:
                logger.warning("Redis unavailable — cannot validate CEO session")
                return False

            raw = await redis.get(f"dashboard_session:{session_id}")
            if raw is None:
                return False

            session = json.loads(raw)

            # Verificar timeout manual (la clave TTL de Redis ya expira la sesión,
            # pero chequeamos last_activity por si el TTL fue extendido)
            last_activity = datetime.fromisoformat(session["last_activity"])
            if datetime.now() - last_activity > self.session_timeout:
                await redis.delete(f"dashboard_session:{session_id}")
                return False

            # Actualizar última actividad (TTL sliding window)
            ...
            logger.warning(f"Redis error validating CEO session {session_id[:8]}...: {e}")
            return False

    async def destroy_session(self, session_id: str):
        """Destruye una sesión CEO en Redis."""
        try:
            redis = get_redis()
            if redis is not None:
                await redis.delete(f"dashboard_session:{session_id}")
        except Exception as e:
            logger.warning(f"Redis error destroying CEO session {session_id[:8]}...: {e}")
...
async def get_ceo_session(request: Request) -> Optional[str]:
    """Obtiene y valida la sesión CEO de la request."""
...
async def ceo_auth_required(request: Request):
    ...
    """
    session_id = await get_ceo_session(request)
