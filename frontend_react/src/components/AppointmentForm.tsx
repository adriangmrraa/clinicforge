                            ...
                            {isEditing && (
                                <div className="space-y-2 p-4 bg-white/[0.03] rounded-xl border border-white/[0.06]">
                                    ...
                                    <div className="grid grid-cols-3 gap-2">
                                        {(() => {
                                            // v8.2: Per-status color map — each status gets its own visible active color
                                            // 500-level shades ensure strong contrast on dark (#0d1117) backgrounds
                                            const STATUS_COLORS: Record<string, string> = {
                                                scheduled: 'bg-blue-500 border-blue-400 ring-blue-300/20',
                                                confirmed: 'bg-emerald-500 border-emerald-400 ring-emerald-300/20',
                                                completed: 'bg-purple-500 border-purple-400 ring-purple-300/20',
                                                cancelled: 'bg-red-500 border-red-400 ring-red-300/20',
                                                no_show:    'bg-slate-500 border-slate-400 ring-slate-300/20',
                                            };
                                            return [
                                                { value: 'scheduled', label: `📅 ${t('agenda.status_scheduled')}` },
                                                { value: 'confirmed', label: `✅ ${t('agenda.status_confirmed')}` },
                                                { value: 'completed', label: `🏁 ${t('agenda.status_completed')}` },
                                                { value: 'cancelled', label: `❌ ${t('agenda.status_cancelled')}` },
                                                { value: 'no_show', label: `👻 ${t('agenda.status_no_show')}` },
                                            ].map(s => (
                                                <button
                                                    key={s.value}
                                                    type="button"
                                                    ...
                                                    className={`px-3 py-2 text-xs font-semibold rounded-lg border transition-all ${
                                                        appointmentStatus === s.value
                                                            ? `${STATUS_COLORS[s.value]} text-white shadow-sm ring-1`
                                                            : 'bg-white/[0.04] border-white/[0.08] text-white/50 hover:bg-white/[0.06]'
